...
- ``address``: Holds a 48 byte value (size of a QRL address).
...
Explicit conversions to and from ``address`` are allowed for ``uint384``, integer literals,
``bytes48`` and contract types.
...
.. warning::
    ...
    You can use ``address(uint384(bytes48(b)))``, which results in ``Q111122223333444455556666777788889999aAaa``,
    or you can use ``address(uint384(uint256(b)))``, which results in ``Q777788889999AaAAbBbbCcccddDdeeeEfFFfCcCc``.
