...
void ImmutableValidator::analyze()
{
	...
	for (ContractDefinition const* contract: linearizedContracts)
		for (ASTPointer<InheritanceSpecifier> const& inheritSpec: contract->baseContracts())
...
{
	// Visit values first as they might access other immutables.
...
void ImmutableValidator::endVisit(IdentifierPath const& _identifierPath)
{
	...
	hypAssert(!dynamic_cast<VariableDeclaration const*>(_identifierPath.annotation().referencedDeclaration), "");
}
...
void ImmutableValidator::endVisit(Identifier const& _identifier)
{
	if (auto const callableDef = dynamic_cast<CallableDeclaration const*>(_identifier.annotation().referencedDeclaration))
		visitCallableIfNew(
			*_identifier.annotation().requiredLookup == VirtualLookup::Virtual ?
			callableDef->resolveVirtual(m_mostDerivedContract) :
			*callableDef
		);
...
void ImmutableValidator::checkAllVariablesInitialized(SourceLocation const& _location)
{
			...
			if (varDecl->immutable())
				if (!util::contains(m_initializedStateVariables, varDecl))
					m_errorReporter.typeError(
						2658_error,
						_location,
						SecondarySourceLocation().append("Not initialized: ", varDecl->location()),
						"Construction control flow ends without initializing all immutable state variables."
					);

		// Do not check further than the current constructor's contract.
...
{
	CallableDeclaration const* callable = dynamic_cast<CallableDeclaration const*>(&_declaration);
	hypAssert(callable != nullptr, "");

	if (m_visitedCallables.emplace(callable).second)
